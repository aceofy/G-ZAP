...
class BF_NIR_conv_Cell(nn.Module):
    def __init__(self, feat_dim=64, guide_dim=64, spa_edsr_num=4, mlp_dim=[256, 128], NIR_dim=32, cell_decode=True):
        ...
        # n_colors=11 (3 Pan + 8 LMS)
        self.spatial_encoder = make_edsr_baseline(n_resblocks=spa_edsr_num, n_feats=self.guide_dim, n_colors=8, no_upsampling=True)
        ...
        self.decoder = nn.Sequential(
            ...
            nn.Conv2d(NIR_dim, 4, kernel_size=5, padding=2, bias=False)
        )
    ...
    def forward(self, HR_MSI, lms, LR_HSI=None, cell=None, target_shape=None):
        """
        HR_MSI: Guide (Pan) [B, 3, H_in, W_in]
