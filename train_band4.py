...
parser.add_argument("--path", type=str, default=r"D:\download\Google_Files\test_gf2_OrigScale_multiExm1.h5",help="H5文件路径")
...
parser.add_argument("--save_dir", type=str, default="results_band4_mat", help="结果保存路径")
...
def save_inference_results(output_tensor, gt_tensor, ms_tensor, pan_tensor, save_dir, data_id, sensor):
    ...
    file_path = os.path.join(save_dir, f"{data_id}_best.mat")
...
def run_batch_process():
    ...
    for curr_id in range(start_id, end_id):
        ...
        with h5py.File(args.path, 'r') as f:
            ...
            pan_0_3c = pan_0.repeat(1, 4, 1, 1) 

        
        with torch.no_grad():
            # Level 1 (Pan 128, MS 32)
            ms_1 = wald_utilities.wald_protocol_v1(ms_0, pan_0, 4, args.sensor, 4)
            pan_1 = wald_utilities.wald_protocol_v2(ms_0, pan_0, 4, args.sensor, 4)
            lms_1 = upsample(ms_1, size=pan_1.shape[-2:]) 
            pan_1_3c = pan_1.repeat(1, 4, 1, 1)

            # Level 2 (Pan 32, MS 8)
            ms_2 = wald_utilities.wald_protocol_v1(ms_1, pan_1, 4, args.sensor, 4)
            pan_2 = wald_utilities.wald_protocol_v2(ms_1, pan_1, 4, args.sensor, 4)
            lms_2 = upsample(ms_2, size=pan_2.shape[-2:])
            pan_2_3c = pan_2.repeat(1, 4, 1, 1)
        ...
        model = BF_NIR_conv_Cell(feat_dim=64, guide_dim=64,spa_edsr_num=3, mlp_dim=[256,128],NIR_dim=48, cell_decode=True).to(device)
        ...
        with torch.no_grad():
            final_out = model(data_dict['L0']['pan'], data_dict['L0']['lms'], None, cell=cell_L0)
            save_inference_results(final_out, data_dict['L0']['lms'], data_dict['L0']['ms'], pan_0, args.save_dir, curr_id, sensor=args.sensor)
